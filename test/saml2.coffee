...
describe 'saml2', ->
  ...
  describe 'private helpers', ->
    ...
    describe 'create_authn_request', ->
      ...
      it 'contains an AuthnContext if requested', ->
        ...
        assert _(requested_authn_context.attributes).some((attr) -> attr.name is 'Comparison' and attr.value is 'exact'),
               "Could not determine if specified attribute had proper value (Comparison=exact)"

        assert.equal requested_authn_context.getElementsByTagNameNS('urn:oasis:names:tc:SAML:2.0:assertion', 'AuthnContextClassRef')[0].firstChild.data,
                     'context:class'
    ...
    describe 'create_metadata', ->
      ...
      it 'contains expected service URLs', ->
        ...
        assert(
          ...
          "Expected to find a SingleLogoutService with redirect binding.")

        assert(
          has_attribute logout_service, 'Location', 'https://sp.example.com/assert',
          "Expected to find a SingleLogoutService with location 'htps://sp.example.com/assert'")
    ...
    describe 'parse_assertion_attributes', ->
      ...
      it 'correctly parses assertion attributes', ->
        expected_attributes = 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname' : [ '' ]
  ...
  describe 'post_assert', ->
      ...
      sp.post_assert idp, request_options, (err, response) ->
        ...
        expected_response =
          ...
          user :
            ...
            session_index : '_3'
            given_name    : 'Test'
            attributes    : 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname': [ 'Test' ]
    ...
    it 'rejects an assertion with a NotBefore condition in the future', (done) ->
      sp_options =
        entity_id        : 'https://sp.example.com/metadata.xml'
        private_key      : get_test_file('test2.pem')
        alt_private_keys : get_test_file('test.pem')
    ...
    it 'rejects an encrypted assertion with a NotBefore condition in the future', (done) ->
      sp_options =
        entity_id        : 'https://sp.example.com/metadata.xml'
        private_key      : get_test_file('test2.pem')
        alt_private_keys : get_test_file('test.pem')
    ...
    it 'accepts an assertion with a NotBefore condition in the future but within the specified skew tolerance', (done) ->
      sp_options =
        entity_id               : 'https://sp.example.com/metadata.xml'
        private_key             : get_test_file('test2.pem')
        alt_private_keys        : get_test_file('test.pem')
    ...
    it 'rejects an assertion with a NotOnOrAfter condition in the past', (done) ->
      sp_options =
        entity_id        : 'https://sp.example.com/metadata.xml'
        private_key      : get_test_file('test2.pem')
        alt_private_keys : get_test_file('test.pem')
    ...
    it 'rejects an encrypted assertion with a NotOnOrAfter condition in the past', (done) ->
      sp_options =
        entity_id        : 'https://sp.example.com/metadata.xml'
        private_key      : get_test_file('test2.pem')
        alt_private_keys : get_test_file('test.pem')
  ...
  describe 'ServiceProvider', ->
    ...
    it 'can create login request url with query paramters', (done) ->
      ...
      async.waterfall [
        ...
        parsed_url   = url.parse login_url, true
        saml_request = parsed_url.query?.SAMLRequest?
        assert.deepEqual _.omit(parsed_url.query, ["SAMLRequest"]), {
    ...
    it 'can create logout request url using a string sso_logout_url', (done) ->
      sp_options =
        entity_id       : 'https://sp.example.com/metadata.xml'
        private_key     : get_test_file('test.pem')
        certificate     : get_test_file('test.crt')
    ...
    it 'can create logout response url using a string sso_logout_url', (done) ->
      sp_options =
        entity_id       : 'https://sp.example.com/metadata.xml'
        private_key     : get_test_file('test.pem')
        certificate     : get_test_file('test.crt')
